#编写程序，并用该程序检查另一个Python程序的代码风格是否符合规范。
#例如运算符两侧是否有空格，是否每次只导入一个模块，在不同的功能模块之间是否有空行等等

import sys
import re

def checkFormats(lines,desFileName):
    fs=open(desFileName,'w')
    for i,line in enumerate(lines):
        ...
        for symbol in symbols:
            ...
            temp_line=pattern.spilt(temp_line)
            sep=''+symbol+''
        ...
        if line.strip() and not line.startswith('') and i >0:
            ...
            if pri_line.strip() and pri_line.startswith(''):
                flag=False
                print(''*10 +"You' better add a blank line before this line.")
                line='\n'+line
        if flag:
            print(''*10+'Pass.')
            fp.write(line)
...
if __name__=='__main__':
    ...
    if ratio <= 0.3:
        ...
        print('Perhaps you should add some cpmments at appropriate position.')
